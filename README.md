...
| `/duplicates` | Compare possible duplicates and choose Same purchase or Separate purchases |
...
Current/new monthly sheets have Date, Time, Amount, Description, Category, Payment Method, Raw Input, Bank Message, hidden Entry ID, Status, Treatment and Source. Each monthly tab contains its overview alongside the transactions in N:O: spending after refunds, pending amounts, possible duplicates, separate investments, a category selector in O2, and a doughnut chart. Selecting a category highlights the matching rows in the original transaction table. A compact chart sits below the overview totals; there is no second copy of the transaction list. All categories clears the highlight. Expand G:I for raw input/bank evidence or K:L for treatment/source; Status stays visible. These are live formulas. Confirmed expenses and returns awaiting refunds enter spending totals; received refunds are excluded, while investments and amounts needing clarification remain visible separately.

Near matches between manual and SMS entries (same day, compatible payment method, within two rupees) are candidates, never proof. Checks run when a manual entry arrives as well as when SMS imports arrive; SMS projection rechecks fresh manual rows before saving. Possible duplicates remain outside spending and show a comparison in Telegram with Same purchase / Separate purchases. Category edits cannot release them, including older category buttons. Separate decisions persist for the identified pair; additional candidates must also be resolved. A confirmed manual/SMS merge keeps the manual description/category, the exact bank amount and both original inputs in one row. A durable merge record recovers interrupted writes. Exact matching SMS references remain idempotent.

Status is visible beside Payment Method in monthly sheets, with possible duplicates highlighted in amber. Raw input/bank evidence (G:I) and internal treatment/source (K:L) remain collapsible. No separate monthly tab or mirrored transaction list is added.
